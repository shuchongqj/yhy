 ...
 * 城市选择侧边字母索引
 * <p>
 * Created by yangboxue on 2018/7/11.
 */

public class CitySelectSideBar extends View {
    ...
    private ArrayList<CityIndexBean> sections;

    private OnTouchSectionListener onTouchSectionListener;

    private boolean isOnTuch = false;

    public CitySelectSideBar(Context context) {
        super(context);
        this.init(context, null);
    }


    public CitySelectSideBar(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.init(context, attrs);
    }


    public CitySelectSideBar(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public CitySelectSideBar(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void init(Context context, AttributeSet attrs) {
        TypedArray typedArray = context.obtainStyledAttributes(attrs,
                R.styleable.CitySelectSideBar);
        this.fontColor = typedArray.getColor(
                R.styleable.CitySelectSideBar_citySelectSideBarFontColor, DEFAULT_FONT_COLOR);
        this.selectBgColor = R.color.color_EA5A57;
        this.viewBackground = typedArray.getColor(
                R.styleable.CitySelectSideBar_citySelectSideBarBackground, DEFAULT_VIEW_BACKGROUND);
        this.touchWrapArea = typedArray.getBoolean(
                R.styleable.CitySelectSideBar_citySelectSideBarTouchWrapArea,
    ...
    private void initPaints() {
        ...
        this.letterSelectPaint = new Paint();
        this.letterSelectPaint.setAntiAlias(true);
        this.letterSelectPaint.setTextAlign(Paint.Align.CENTER);
        this.letterSelectPaint.setColor(Color.WHITE);
        this.letterSelectPaint.setTextSize(this.sectionFontSize);

        this.selectBgPaint = new Paint();
        this.selectBgPaint.setAntiAlias(true);
        this.selectBgPaint.setColor(Color.parseColor("#E50011"));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (this.sections.size() > 0) {
            ...
            for (int i = 0; i < this.sections.size(); i++) {
                CityIndexBean section = this.sections.get(i);
                ...
                if (sections.get(i).isSelect && i != 0) {
                    // 画被选中的背景
                    RectF rectF = new RectF((viewWidth - sectionFontSize) / 2 - sectionFontSize / 12, top + this.sectionHeight * (i - 1) + sectionFontSize / 2 - sectionFontSize / 12, (viewWidth + sectionFontSize) / 2 + sectionFontSize / 12, top + this.sectionHeight * (i - 1) + sectionFontSize / 2 + sectionFontSize + sectionFontSize / 12);
                    canvas.drawOval(rectF, selectBgPaint);

                    canvas.drawText(section.name, viewHalfWidth, top + this.sectionHeight * i,
                            this.letterSelectPaint);
                } else {
                    canvas.drawText(section.name, viewHalfWidth, top + this.sectionHeight * i,
                            this.letterPaint);
                }
...
//                    sections.get(i).isSelect = (i == sectionIndex);
//                }
//                invalidate();
//            }
...
//            }
//            for (int i = 0; i < sections.size(); i++) {
//                sections.get(i).isSelect = sectionIndex == i;
//            }
//            invalidate();
...
//                invalidate();
    ...
    private void showFloatView(float eventY) {
        ...
        CityIndexBean section = this.sections.get(sectionIndex);
        if (this.onTouchSectionListener == null) {
            return;
        }
    ...
    public void setSections(String[] sections) {
        ...
        for (String section : sections) {
            this.sections.add(new CityIndexBean());
        }
        this.invalidate();
    }


    public void setSections(List<CityIndexBean> sections) {
        this.sections.clear();
...
//
    ...
    public interface OnTouchSectionListener {
         ...
         */
        void onTouchLetterSection(int sectionIndex, CityIndexBean letterSection);
    }
    ...
    public void setSelectIndex(int position) {
        if (sections != null || sections.size() > 0) {
            for (int i = 0; i < sections.size(); i++) {
                if (sections.get(i).isSelect && position == i) {
                    return;    // 选中位置不变的化不做操作
                }
                sections.get(i).isSelect = (i == position);
            }
            invalidate();
        }
    }

    public boolean isOnTuch() {
        return isOnTuch;
    }
}
