...
import com.yhy.boombutton.Animation.*;
import com.yhy.boombutton.BoomButtons.*;
import com.yhy.boombutton.Piece.*;

import java.util.ArrayList;

import static com.yhy.boombutton.Piece.PiecePlaceEnum.Share;
...
public class BoomMenuButton extends FrameLayout implements InnerOnBoomButtonClickListener {
    ...
    private void dimBackground(boolean immediately) {
        ...
        Util.setVisibility(VISIBLE, background.getView());
    ...
    private void finishReboomAnimations() {
        ...
        background.getView().setVisibility(GONE);
        clearViews(false);
    }
    ...
    private void createBackground() {
        background = onCreateBackground(context, this);
        if (background == null) {
    ...
    public ViewGroup getParentView() {
        if (boomInWholeScreen) {
            ...
            if (getParent().getParent().getParent() != null) {
                return (ViewGroup) getParent().getParent().getParent().getParent();
            } else {
                return (ViewGroup) getParent();
            }
        }
    ...
    private void clearBackground() {
        ...
        Util.setVisibility(GONE, background.getView());
        if (!cacheOptimization || inList || inFragment) {
            background.removeAllViews();
            ((ViewGroup)background.getParent()).removeView(background.getView());
