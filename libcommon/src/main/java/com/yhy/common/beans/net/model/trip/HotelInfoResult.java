...
public class HotelInfoResult implements Serializable{
     ...
     */
    public static HotelInfoResult deserialize(String json) throws JSONException {
     ...
     */
    public static HotelInfoResult deserialize(JSONObject json) throws JSONException {
        if (json != null && json != JSONObject.NULL && json.length() > 0) {
            HotelInfoResult result = new HotelInfoResult();
            ...
            if (listArray != null) {
                ...
                result.list = new ArrayList<HotelInfo>(len);
                for (int i = 0; i < len; i++) {
                    JSONObject jo = listArray.optJSONObject(i);
                    if (jo != null && jo != JSONObject.NULL && jo.length() > 0) {
                        result.list.add(HotelInfo.deserialize(jo));
                    }
                }
            }
