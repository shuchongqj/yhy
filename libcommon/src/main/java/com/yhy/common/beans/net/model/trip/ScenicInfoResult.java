...
public class ScenicInfoResult implements Serializable{
     ...
     */
    public static ScenicInfoResult deserialize(String json) throws JSONException {
     ...
     */
    public static ScenicInfoResult deserialize(JSONObject json) throws JSONException {
        if (json != null && json != JSONObject.NULL && json.length() > 0) {
            ScenicInfoResult result = new ScenicInfoResult();
            ...
            if (listArray != null) {
                ...
                result.list = new ArrayList<ScenicInfo>(len);
                for (int i = 0; i < len; i++) {
                    JSONObject jo = listArray.optJSONObject(i);
                    if (jo != null && jo != JSONObject.NULL && jo.length() > 0) {
                        result.list.add(ScenicInfo.deserialize(jo));
                    }
                }
            }
    ...
    public JSONObject serialize() throws JSONException {
        ...
        if (this.list != null) {
            ...
            for (ScenicInfo value : this.list)
            {
                if (value != null) {
