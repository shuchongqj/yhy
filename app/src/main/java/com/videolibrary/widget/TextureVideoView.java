 ...
 * Title:TextureVideoView
 * Description:This is player implementation based on {@link TextureView}
...
//            am.abandonAudioFocus(null);
    ...
    public void onCompletion(final MediaPlayer mp) {
        if(SHOW_LOGS) Log.d(TAG, "onCompletion() called");
    ...
    public boolean onError(final MediaPlayer mp, final int what, final int extra) {
        if(SHOW_LOGS) Log.d(TAG, "onError() called with " + "mp = [" + mp + "], what = [" + what + "], extra = [" + extra + "]");
        mCurrentState = STATE_ERROR;
        mTargetState = STATE_ERROR;
        if (mMediaPlayerCallback != null) {
            mHandler.post(new Runnable() {
    ...
    public void onVideoSizeChanged(final MediaPlayer mp, final int width, final int height) {
        if(SHOW_LOGS) Log.d(TAG, "onInfo() called with " + "mp = [" + mp + "], width = [" + width + "], height = [" + height + "]");
        if (mMediaPlayerCallback != null) {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
    ...
    public void onBufferingUpdate(final MediaPlayer mp, final int percent) {
        if(SHOW_LOGS) Log.d(TAG, "onInfo() called with " + "mp = [" + mp + "], percent = [" + percent + "]");
        if (mMediaPlayerCallback != null) {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
    ...
    public boolean onInfo(final MediaPlayer mp, final int what, final int extra) {
        if(SHOW_LOGS) Log.d(TAG, "onInfo() called with " + "mp = [" + mp + "], what = [" + what + "], extra = [" + extra + "]");
        if (mMediaPlayerCallback != null) {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
