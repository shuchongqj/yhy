package com.videolibrary.chat.callback;
...
public class LiveChatSocketHandler extends SimpleChannelHandler {

    private Logger logger = Logger.getLogger(LiveChatSocketHandler.class);

    @Override
    public void channelConnected(ChannelHandlerContext ctx, ChannelStateEvent e)
            throws Exception {
        super.channelConnected(ctx, e);
        logger.d("livechat channel#channelConnected");
        LiveChatSocketManager.instance().onMsgServerConnected();
    }
    ...
    public void channelDisconnected(ChannelHandlerContext ctx,
         ...
         *
         **/
        logger.d("livechat channel#channelDisconnected");
    ...
    public void messageReceived(ChannelHandlerContext ctx, MessageEvent e)
        ...
        logger.d("livechat channel#messageReceived");
        // 重置AlarmManager的时间
        ChannelBuffer channelBuffer = (ChannelBuffer) e.getMessage();
        if (null != channelBuffer)
            LiveChatSocketManager.instance().packetDispatch(channelBuffer);
    }
    ...
    public void exceptionCaught(ChannelHandlerContext ctx, ExceptionEvent e) throws Exception {
        ...
        if (e.getChannel() == null || !e.getChannel().isConnected()) {
            LiveChatSocketManager.instance().onConnectMsgServerFail();
        }
        logger.e("livechat channel#[网络异常了]exceptionCaught:%s", e.getCause().toString());
    }
}
