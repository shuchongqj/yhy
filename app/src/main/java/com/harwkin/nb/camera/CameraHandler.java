...
public class CameraHandler implements CameraOperate {
    ...
    public void onOpenUserAlbum(Intent intent) {
        if (mContext instanceof Activity) {
            ((Activity) mContext).startActivityForResult(intent, CameraManager.OPEN_USER_ALBUM);
        } else {
            throw new IllegalArgumentException(" Context need activity ");
        }
    }
    ...
    public void processWithMedia() throws ClassNotFoundException {
        mCameraManager.getImageAndVideoFromUserAlbum();
    }

    public void processWithVideo() throws ClassNotFoundException {
        mCameraManager.getVideoFromUserAlbum();
    }
