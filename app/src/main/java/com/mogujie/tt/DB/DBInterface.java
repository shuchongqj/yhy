...
public class DBInterface {
    ...
    public void initDbHelp(Context ctx, long loginId) {
    ...
    private void isInitOk() {
        if (openHelper == null) {
            ...
            // 抛出异常 todo
            if (context == null) context = YHYBaseApplication.getInstance();
            if (userService.isLogin()) {
                DBInterface.instance().initDbHelp(context, userService.getLoginUserId());
            }
        }
    }
    ...
    public List<MessageEntity> getHistoryMsg(String chatKey, int lastMsgId, int lastCreateTime, int count) {
        ...
        List<MessageEntity> listMsg = dao.queryBuilder().where(MessageDao.Properties.SessionKey.eq(chatKey)
    ...
    public void setMessageStatusFailed() {
        MessageDao dao = openWritableDb().getMessageDao();
        List<MessageEntity> entities = dao.queryBuilder().where(MessageDao.Properties.Status.eq(MessageConstant.MSG_SENDING)).listLazyUncached();
        for (MessageEntity entity : entities) {
            entity.setStatus(MessageConstant.MSG_FAILURE);
        }
    ...
    public void deleteMessageById(long localId) {
        ...
        openWritableDb().getMessageDao().deleteByKey(localId);
    }
//
...
//    }
    ...
    public void deleteMessageByMsgId(int msgId) {
        ...
        DeleteQuery<MessageEntity> bd = messageDao.queryBuilder()
                .where(MessageDao.Properties.MsgId.eq(msgId)).buildDelete();
    ...
    private MessageEntity formatMessage(MessageEntity msg) {
        ...
        switch (displayType) {
            ...
            case DBConstant.SHOW_PRODUCT_CARD_TYPE:
                messageEntity = ProductCardMessage.parseFromDB(msg);
                break;
            default:
                messageEntity = UnknownMessage.parseFromDB(msg);
        }
        ...
        for (MessageEntity info : msgList) {
            ...
            switch (displayType) {
                ...
                case DBConstant.SHOW_ORIGIN_TEXT_TYPE:
                    ...
                    break;
                case DBConstant.SHOW_PRODUCT_CARD_TYPE:
                    newList.add(ProductCardMessage.parseFromDB(info));
                    break;
                case DBConstant.SHOW_NOTIFY:
                    newList.add(NotifyMessage.parseFromDB(info));
                    break;
                case DBConstant.SHOW_KNOWLEDGE_TYPE:
                    newList.add(KnowLedgeMessage.parseFromDB(info));
                    break;
                default:
                    newList.add(UnknownMessage.parseFromDB(info));
            }
        }
        return newList;
    }
