...
public class IMGroupAvatar extends LinearLayout {
    ...
    private int defaultChildAvatarRes = 0;
    private int defaultParentAvatarBk = 0;
    ...
    public IMGroupAvatar(Context context, AttributeSet attrs) {
        ...
        parentPadding = AndroidUtils.dip2px(getContext(), DEFAULT_PADDING_DIP);
        childMargin = AndroidUtils.dip2px(getContext(), DEFAULT_MARGIN_DIP);
        parentAvatarSize = AndroidUtils.dip2px(getContext(), DEFAULT_VIEW_SIZE_DIP);
        setPadding(parentPadding, parentPadding, parentPadding, parentPadding);
    }

    public void setParentPadding(int padding){
        if (padding >= 0) {
            parentPadding = AndroidUtils.dip2px(getContext(), padding);
            setPadding(parentPadding, parentPadding, parentPadding, parentPadding);
        }
    }
    ...
    private void initOneRow(int curRow,boolean isFirst, int columnSize){
        try {
            ...
            for (int i = 0; i < columnSize; i++) {
                ...
                avatar.setImageResource(R.mipmap.icon_default_avatar);
                avatar.setCorner(AndroidUtils.dip2px(getContext(), childCorner));
