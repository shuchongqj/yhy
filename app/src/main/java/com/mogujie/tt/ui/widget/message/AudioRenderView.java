...
public class AudioRenderView extends BaseMsgRenderView {
    ...
    public void render(final MessageEntity messageEntity, final UserEntity userEntity, final Context ctx) {
        ...
        if (null != audioPath) {
            ...
            if (width < AndroidUtils.dip2px(getContext(), 45)) {
                width = AndroidUtils.dip2px(getContext(), 45);
            }
            LinearLayout.LayoutParams layoutParam = new LinearLayout.LayoutParams(width, LayoutParams.WRAP_CONTENT);
...
//            }
    ...
    public void playAudio(AudioMessage audioMessage, Context ctx) {
        final String audioPath = audioMessage.getAudioPath();
        int audioReadStatus = audioMessage.getReadStatus();
