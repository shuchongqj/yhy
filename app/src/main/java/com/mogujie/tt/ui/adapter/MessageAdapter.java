    ...
    public void addItem(final MessageEntity msg) {
        ...
        if (getCount() > 0) {
            ...
            if (object instanceof MessageEntity) {
                ...
                if (msg.getMsgType() != DBConstant.MSG_TYPE_CONSULT_NOTIFY && needTime) {
                    Integer in = nextTime;
...
//        return Emoparser.getInstance(this.ctx).isMessageGif(msg.getContent());
    ...
    public void loadHistoryList(final List<MessageEntity> historyList) {
        ...
        for (MessageEntity msg : historyList) {
            ...
            if (msg.getMsgType() != DBConstant.MSG_TYPE_CONSULT_NOTIFY && needTimeBubble) {
                Integer in = nextTime;
                chatList.add(in);
            }
            preTime = nextTime;
...
//    public void clearImageMessageList(){
//        imageMessageList.clear();
//    }
            ...
            if (obj instanceof Integer) {
                ...
                switch (info.getDisplayType()) {
                    ...
                    case DBConstant.SHOW_IMAGE_TYPE:
                        ...
                        String path = ((ImageMessage) info).getPath();
//                        if ((!StringUtil.isEmpty(path) && path.toLowerCase().endsWith(".gif")) || CommonUtil.gifCheck(imageMessage.getUrl())) {
...
//                        }
                    ...
                    case DBConstant.SHOW_PRODUCT_CARD_TYPE:
                        type = isMine ? RenderType.MESSAGE_TYPE_MINE_PRODUCT_CARD : RenderType.MESSAGE_TYPE_OTHER_PRODUCT_CARD;
                        break;
                    case DBConstant.SHOW_NOTIFY:
                        type = RenderType.MESSAGE_TYPE_NOTIFY;
                        break;
                    case DBConstant.SHOW_KNOWLEDGE_TYPE:
                        type = isMine ? RenderType.MESSAGE_TYPE_MINE_KNOWLEDGE : RenderType.MESSAGE_TYPE_OTHER_KNOWLEDGE;
                        break;
                    case DBConstant.SHOW_MIX_TEXT:
                        //
                        logger.e("混合的消息类型%s", obj);
                    default:
                        type = isMine ? RenderType.MESSAGE_TYPE_MINE_UNKNOWN : RenderType.MESSAGE_TYPE_OTHER_UNKNOW;
                        break;
                }
            }
            return type.ordinal();
     ...
     * 通知消息类型,ui与时间通用
     */
    private View notifyMsgRender(int position, View convertView, ViewGroup parent) {
        TimeRenderView timeRenderView;
        NotifyMessage message = (NotifyMessage) msgObjectList.get(position);
        if (null == convertView) {
            timeRenderView = TimeRenderView.inflater(ctx, parent);
        } else {
            // 不用再使用tag 标签了
        ...
        timeRenderView.setContent(message.getContent());
        return timeRenderView;
    }
    ...
    private View imageMsgRender(final int position, View convertView, final ViewGroup parent, final boolean isMine) {
        ...
        imageRenderView.setBtnImageListener(new ImageRenderView.BtnImageListener() {
                                                ...
                                                public void onMsgFailure() {
                                                     ...
                                                     */
//                resend(imageMessage,position);
                                                    if (!FileUtil.isFileExist(imagePath) && !FileUtil.isFileExist(imageMessage.getThumbnailUrl())) {
                                                    ...
                                                    pics.add(imagePath);
                                                    NavUtils.gotoLookBigImage(ctx, pics, 0, false);
                                                }
...
//        );
        ...
        final boolean bResend = (imageMessage.getStatus() == MessageConstant.MSG_FAILURE)
...
//            });
//        } else {
//            messageImage.setOnLongClickListener(null);
//        }

        /**父类控件中的发送失败view*/
        imageRenderView.getMessageFailed().
    ...
    private View audioMsgRender(final int position, View convertView, final ViewGroup parent, final boolean isMine) {
        final AudioRenderView audioRenderView;
        ...
        if (!TextUtils.isEmpty(audioPath)) {
            ...
            messageLayout.setOnLongClickListener(new View.OnLongClickListener() {
                @Override
                public boolean onLongClick(View v) {
                    MessageOperatePopup popup = getPopMenu(parent, new OperateItemClickListener(audioMessage, position, audioRenderView));
...
//        });
    ...
    private View unKnownMsgRender(final int position, View convertView, final ViewGroup viewGroup, final boolean isMine) {
        TextRenderView textRenderView;
        final UnknownMessage unKnownMessage = (UnknownMessage) msgObjectList.get(position);
        UserEntity userEntity = imService.getContactManager().findContact(unKnownMessage.getFromId());
        if (null == convertView) {
            textRenderView = TextRenderView.inflater(ctx, viewGroup, isMine); //new TextRenderView(ctx,viewGroup,isMine);
        } else {
            textRenderView = (TextRenderView) convertView;
        }
        textRenderView.render(unKnownMessage, userEntity, ctx);
        return textRenderView;
    }
        ...
        try {
            ...
            switch (renderType) {
                ...
                case MESSAGE_TYPE_NOTIFY:
                    convertView = notifyMsgRender(position, convertView, parent);
                    break;
                ...
                case MESSAGE_TYPE_MINE_UNKNOWN:
                    convertView = unKnownMsgRender(position, convertView, parent, true);
                    break;
                case MESSAGE_TYPE_OTHER_UNKNOW:
                    convertView = unKnownMsgRender(position, convertView, parent, false);
                    break;
...
//                    break;
                case MESSAGE_TYPE_MINE_PRODUCT_CARD:
                    convertView = productCardMsgRender(position, convertView, parent, true);
                    break;
                case MESSAGE_TYPE_OTHER_PRODUCT_CARD:
                    convertView = productCardMsgRender(position, convertView, parent, false);
                    break;
                case MESSAGE_TYPE_MINE_KNOWLEDGE:
                    convertView = knowledgeMsgRender(position, convertView, parent, true);
                    break;
                case MESSAGE_TYPE_OTHER_KNOWLEDGE:
                    convertView = knowledgeMsgRender(position, convertView, parent, false);
            }
            return convertView;
        } catch (Exception e) {
            logger.e("chat#%s", e);
     ...
     * @return
     */
    private View knowledgeMsgRender(final int position, View convertView, final ViewGroup viewGroup, final boolean isMine) {
        KnowledgeRenderView knowledgeRenderView;
        final KnowLedgeMessage textMessage = (KnowLedgeMessage) msgObjectList.get(position);
        UserEntity userEntity = imService.getContactManager().findContact(textMessage.getFromId());
        if (null == convertView) {
            knowledgeRenderView = KnowledgeRenderView.inflater(ctx, viewGroup, isMine); //new TextRenderView(ctx,viewGroup,isMine);
        } else {
            knowledgeRenderView = (KnowledgeRenderView) convertView;
        }
        knowledgeRenderView.render(textMessage, userEntity, ctx);
     ...
     * @return
     */
    private View productCardMsgRender(final int position, View convertView, final ViewGroup parent, final boolean isMine) {
        ProductCardRenderView productCardRenderView;
        final ProductCardMessage productCardMessage = (ProductCardMessage) msgObjectList.get(position);
        UserEntity entity = imService.getContactManager().findContact(productCardMessage.getFromId());
        if (null == convertView) {
            productCardRenderView = ProductCardRenderView.inflater(ctx, parent, isMine);
        } else {
            productCardRenderView = (ProductCardRenderView) convertView;
        }

        productCardRenderView.render(productCardMessage, entity, ctx);
        final View messageLayout = productCardRenderView.getMessageLayout();

        // 失败事件添加
        productCardRenderView.getMessageFailed().setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View arg0) {
                resend(productCardMessage, position);
            }
        });
        if (productCardMessage.getStatus() == MessageConstant.MSG_FAILURE && isMine) {
            messageLayout.setOnLongClickListener(new View.OnLongClickListener() {
                @Override
                public boolean onLongClick(View v) {
                    // 弹窗类型
                    MessageOperatePopup popup = getPopMenu(parent, new OperateItemClickListener(productCardMessage, position));
                    popup.show(messageLayout, DBConstant.SHOW_PRODUCT_CARD_TYPE, true, isMine);
                    return true;
                }
            });
        } else {
        ...
        messageLayout.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                NavUtils.gotoProductCardDetails(ctx, productCardMessage);
            }
        });
    ...
    private MessageOperatePopup getPopMenu(ViewGroup parent, MessageOperatePopup.OnItemClickListener listener) {
        MessageOperatePopup popupView = new MessageOperatePopup(ctx);
    ...
    private class OperateItemClickListener
        ...
        public OperateItemClickListener(MessageEntity mMsgInfo, int mPosition, BaseMsgRenderView renderView) {
            this(mMsgInfo, mPosition);
        ...
        public void onDeleteClick() {
            DBInterface.instance().deleteMessageById(mMsgInfo.getId());
            if (mPosition == msgObjectList.size() - 1) {
                boolean isUpdate = false;
                for (int i = mPosition - 1; i >= 0; i--) {
                    if (msgObjectList.get(i) instanceof Integer) {
                        continue;
                    } else {
//                        imService.getSessionManager().updateSession((MessageEntity) msgObjectList.get(i));
                        isUpdate = true;
                        break;
                    }
                }
                if (!isUpdate) {
                    mMsgInfo.setContent("");
//                    imService.getSessionManager().updateSession(mMsgInfo);
                }
                if (msgObjectList.get(mPosition - 1) instanceof Integer) {
                    msgObjectList.remove(mPosition - 1);
                }
            } else if (mPosition != 0) {
                if (msgObjectList.get(mPosition - 1) instanceof Integer && msgObjectList.get(mPosition + 1) instanceof Integer) {
                    msgObjectList.remove(mPosition - 1);
                }
            }
            msgObjectList.remove(mMsgInfo);
            notifyDataSetChanged();
        }
        ...
        public void onSpeakerClick() {
            ...
            if (streamType == AudioManager.STREAM_MUSIC) {
                SPUtils.saveAudioStreamType(ctx, AudioManager.STREAM_VOICE_CALL);
                ToastUtil.showToast(ctx, R.string.audio_in_call);
//                SpeekerToast.show(ctx, ctx.getText(R.string.audio_in_call), Toast.LENGTH_SHORT);
            } else {
                SPUtils.saveAudioStreamType(ctx, AudioManager.STREAM_MUSIC);
                ToastUtil.showToast(ctx, R.string.audio_in_speeker);
//                SpeekerToast.show(ctx, ctx.getText(R.string.audio_in_speeker), Toast.LENGTH_SHORT);
            }
            if (renderView != null && renderView instanceof AudioRenderView) {
                ((AudioRenderView) renderView).playAudio((AudioMessage) mMsgInfo, ctx);
            }
        }

    }

    private void resend(MessageEntity mMsgInfo, int mPosition) {
        int mType = mMsgInfo.getDisplayType();
        try {
            if (mType == DBConstant.SHOW_AUDIO_TYPE
                    || mType == DBConstant.SHOW_ORIGIN_TEXT_TYPE) {
