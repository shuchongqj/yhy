...
//                    IMGroupManager.instance().receiveGroupChangeMemberNotify(notify);
    ...
    public static void switchServiceDispatcher(int commandId, CodedInputStream codedInputStream) {
        try {
            switch (commandId) {
                case IMBaseDefine.SwitchServiceCmdID.CID_SWITCH_P2P_CMD_VALUE:
                    IMSwitchService.IMP2PCmdMsg msg = IMSwitchService.IMP2PCmdMsg.parseFrom(codedInputStream);
                    IMSwitchServiceManager.instance().onRecP2PMsg(msg);
            }
        } catch (Exception e) {
    ...
    public static void controlPacketDispatcher(int commandId, CodedInputStream codedInputStream) {
        try {
            switch (commandId) {
                case IMBaseDefine.MessageCmdID.CID_MSG_GEEK_CONTROL_VALUE:
                    IMMessage.IMGeekControl geekControl = IMMessage.IMGeekControl.parseFrom(codedInputStream);
                    String content = geekControl.getMsgData().toStringUtf8();
                    JSONObject jsonObject = new JSONObject(content);
                    String topic = jsonObject.optString(ConsultContants.TOPIC);
                    if (ConsultContants.CONSULTION.equals(topic)) {
                        IMConsultManager.instance().dispatchControllMessage(content);
                    }
            }
        } catch (IOException e) {
            logger.e("msgPacketDispatcher# error,cid:%d", commandId);
    ...
    public static void otherPacketDispatcher(int commandId, CodedInputStream codedInputStream) {
        try {
            switch (commandId) {
                case IMBaseDefine.OtherCmdID.CID_OTHER_CAR_INFO_REP_VALUE:
                    IMOther.IMCarInfoRep rep = IMOther.IMCarInfoRep.parseFrom(codedInputStream);
                    IMOtherManager.instance().onRecCarInfo(rep);
                    break;
            }
        } catch (IOException e) {
            logger.e("otherPacketDispatcher# error,cid:%d", commandId);
        }
    }
