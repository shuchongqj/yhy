...
public class IMSocketManager extends IMManager {
     ...
     * 快速重新连接的时候需要
     */
    private Api_USER_ImLoginResp currentMsgAddress = null;
    ...
    public void doOnStart() {
        ...
        SslContextFactory.getInstance().init(ctx);
    }
    ...
    public void packetDispatch(ChannelBuffer channelBuffer) {
        ...
        switch (serviceId) {
            ...
            case IMBaseDefine.ServiceID.SID_GROUP_VALUE:
                ...
                break;
            case IMBaseDefine.ServiceID.SID_CONTROL_VALUE:
                IMPacketDispatcher.controlPacketDispatcher(commandId, codedInputStream);
                break;
            case IMBaseDefine.ServiceID.SID_SWITCH_SERVICE_VALUE:
                IMPacketDispatcher.switchServiceDispatcher(commandId,codedInputStream);
                break;
...
//    }
    ...
    public void reqImMsgServerAddrs() {
        triggerEvent(SocketEvent.REQING_MSG_SERVER_ADDRS);

        NetManager.getInstance(ctx).doGetImServerAddrs(new OnResponseListener<Api_USER_ImLoginResp>() {
            @Override
            public void onComplete(boolean isOK, Api_USER_ImLoginResp result, int errorCode, String errorMsg) {
                logger.d("reqest imMsgServerAddrs complete isok : %b", isOK);
                if (isOK) {
                    triggerEvent(SocketEvent.REQ_MSG_SERVER_ADDRS_SUCCESS);
                    IMLoginManager.instance().setImToken(result.imToken);
    ...
    private void connectMsgServer(Api_USER_ImLoginResp currentMsgAddress) {
...
//            } else {
            //屏蔽直接连入im服务器，请求中台imlogin接口，来验证token的有效性，用来显示踢人
            disconnectMsgServer();
