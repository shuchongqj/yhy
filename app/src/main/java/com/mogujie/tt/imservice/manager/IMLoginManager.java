...
public class IMLoginManager extends IMManager {
    ...
    public void doOnStart() {
        if (userService.isLogin()) {
            login(userService.getLoginUserId());
        }
    ...
    public void relogin() {
        if (loginId > 0) {
            ...
            imSocketManager.reqImMsgServerAddrs();
        } else {
    ...
    public void login(long loginId) {
        ...
        // 初始化数据库
        DBInterface.instance().initDbHelp(ctx, loginId);
        UserEntity loginEntity = DBInterface.instance().getByLoginId(loginId);
        do {
            if (loginEntity == null) {
                loginEntity= new UserEntity();
                int timeNow = (int) (System.currentTimeMillis() / 1000);
                String nickname = SPUtils.getNickName(ctx);
                loginEntity.setAvatar(ImageUtils.getImageFullUrl(SPUtils.getUserIcon(ctx)));
                loginEntity.setCreated(timeNow);
                loginEntity.setPinyinName(nickname);
                loginEntity.setGender(1);
                loginEntity.setMainName(nickname);
                loginEntity.setUpdated(timeNow);
                loginEntity.setPeerId(userService.getLoginUserId());
                loginEntity.setOptions((int) SPUtils.getRoleType(ctx));
                loginEntity.setIsVip(SPUtils.isVip(ctx));
                DBInterface.instance().insertOrUpdateUser(loginEntity);
            }
    ...
    public void reqLoginMsgServer() {
        ...
        IMLogin.IMLoginReq imLoginReq = IMLogin.IMLoginReq.newBuilder().setUserId(loginId)
                .setAppId(Integer.parseInt(ContextHelper.getAppId()))
                .setDomainId(ContextHelper.getDomainId())
                .setUserToken(imToken)
                .setOnlineStatus(IMBaseDefine.UserStatType.USER_STATUS_ONLINE)
                .setClientVersion(AndroidUtils.getVersion(ctx)).build();
...
//        reqDeviceToken();
    ...
    private void checkServiceStatus(){
        logger.i("login#checkServiceStatusStart");
        if(userService.isLogin() && SPUtils.isInWhiteList(ctx) > 0 && SPUtils.isManualUpdateServiceStatus(ctx)){
            NetManager.getInstance(ctx).doEditUserStatus(userService.getLoginUserId(), UserStatus.ONLINE,new OnResponseListener<Boolean>() {
                @Override
                public void onComplete(boolean isOK, Boolean result, int errorCode, String errorMsg) {
                    if(isOK){
                        logger.i("login#ServiceStartOK");
                        return;
                    }
                    logger.i("login#ServiceStartFailed");
                }
...
//    }
    ...
    public void onLoginStatusNotify(IMBuddy.IMPCLoginStatusNotify statusNotify) {
        long userId = statusNotify.getUserId();
        // todo 由于交互不太友好 暂时先去掉
