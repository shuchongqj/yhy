...
public class MsgAnalyzeEngine {
    ...
    public static MessageEntity analyzeMessage(IMBaseDefine.MsgInfo msgInfo) {
        ...
        if (!msgInfo.hasExtInfo()) {
            msgType = ProtoBuf2JavaBean.getJavaMsgType(msgInfo.getMsgType());
            content = msgInfo.getMsgData().toStringUtf8();
        } else {
            IMBaseDefine.ExtInfo extInfo = msgInfo.getExtInfo();
            msgType = ProtoBuf2JavaBean.getJavaMsgTypeNew(extInfo.getMsgType());
        ...
        messageEntity.setServiceId(msgInfo.getMsgItem());
        /**
         * 解密文本信息
         */
