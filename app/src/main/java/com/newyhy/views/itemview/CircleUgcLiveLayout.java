...
/**
 * CircleLiveLayout 圈子中直播 Item 的布局
 * Created by Jiervs on 2018/6/20.
 */

public class CircleUgcLiveLayout extends LinearLayout {
    ...
    private TextView tv_support_num, tv_location;
    ...
    public CircleUgcLiveLayout(Context context) {
        super(context);
        initViews(context);
    }

    public CircleUgcLiveLayout(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        initViews(context);
    }

    public CircleUgcLiveLayout(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    public void initViews(Context context) {
        ...
        LayoutInflater.from(context).inflate(R.layout.circle_ugc_live_layout, this);
    ...
    public <T> void covertData(T t) {
        if (t instanceof UgcInfoResult) {//UgcInfoResult
            ...
            if (data.poiInfo != null) {
                location = data.poiInfo.detail;
            }
            id = data.id;
    ...
    public <T> void setData(Context context, T data, boolean isDetail) {
        ...
        if (isDetail) {
            ...
            if (picList != null) {
                if (picList.size() > 0) {
                    ImageLoadManager.loadImage(CommonUtil.getImageFullUrl(picList.get(0)), R.mipmap.icon_default_750_360, iv_cover);
                }
            }
        ...
        ll_comment.setOnClickListener(v -> {
            if (userService.isLogin()) {
                IntentUtil.startVideoClientActivity(liveId, 0, liveStatus == 1, screenType);
            } else {
    ...
    public void doPlay() {
        ...
        new ActivityCenterApi().play(new PlayReq(new PlayReq.Companion.P(liveId, CircleBizType.SNS_VEDIO_SHORT)), callback).execAsync();
    }
}
