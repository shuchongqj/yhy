...
public abstract class NineGridLayout extends ViewGroup {
    ...
    private void refresh() {
        ...
        if (size == 1) {
            ...
            RoundImageView imageView = createRoundImageView(0, url);
            ...
            if (isShowDefualt) {
                layoutRoundImageView(imageView, 0, url, false);
            } else {
                addView(imageView);
            }
            return;
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (!mIsShowAll) {
                if (i < MAX_COUNT - 1) {
                    imageView = createRoundImageView(i, url);
                    layoutRoundImageView(imageView, i, url, false);
                } else { //第9张时
                    if (size <= MAX_COUNT) {//刚好第9张
                        ...
                        imageView = createRoundImageView(i, url);
                        layoutRoundImageView(imageView, i, url, true);
                        break;
                    }
                }
            } else {
    ...
    private RoundImageView createRoundImageView(final int i, final String url) {
        RoundImageView imageView = new RoundImageView(mContext);
        imageView.setScaleType(RoundImageView.ScaleType.CENTER_CROP);
        imageView.setBorderRadius(2);
     ...
     */
    private void layoutRoundImageView(RoundImageView imageView, int i, String url, boolean showNumFlag) {
        ...
        if (url.toLowerCase().endsWith(".gif")) {//贴动态图标签
            final TextView tv_gif = new TextView(mContext);
            int w = (int) mContext.getResources().getDimension(R.dimen.yhy_size_30px);
            int h = (int) mContext.getResources().getDimension(R.dimen.yhy_size_18px);
            LayoutParams params = new LayoutParams(w,h);
            tv_gif.setLayoutParams(params);
            tv_gif.setText(mContext.getResources().getString(R.string.gif));
            tv_gif.setTextColor(Color.WHITE);
            tv_gif.setTextSize(12);
            tv_gif.setGravity(Gravity.CENTER);
            tv_gif.setBackgroundColor(mContext.getResources().getColor(R.color.tv_color_gray5));
            tv_gif.getBackground().setAlpha(220);
            tv_gif.layout(right - w, bottom - h, right, bottom);
            addView(tv_gif);
        }
    }
    ...
    protected void setOneImageLayoutParams(RoundImageView imageView, int width, int height) {
...
//        params.width = width;
     ...
     */
    protected abstract boolean displayOneImage(RoundImageView imageView, String url, int parentWidth);

    protected abstract void displayImage(RoundImageView imageView, String url);
