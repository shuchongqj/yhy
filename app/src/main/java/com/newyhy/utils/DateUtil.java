...
public class DateUtil {
    ...
    private static final long DAY_SECONDS = 60 * 60 * 24;//超过24个小时算一天
    ...
    public static String getCreateAt(long second) throws FormatException {
        ...
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd",new Locale("CN"));
        SimpleDateFormat noDayFormat = new SimpleDateFormat("HH:mm",new Locale("CN"));
        long dayFromData = getDayEndTimer(second);
        long dayCurrent = getDayEndTimer(System.currentTimeMillis());
        long offsetDay = (dayCurrent - dayFromData) / 1000;

        if (offsetDay > DAY_SECONDS * 9) {
            return simpleDateFormat.format(new Date(second));
        } else if (offsetDay == DAY_SECONDS * 9 ){
            return "9天前 "; //+ noDayFormat.format(new Date(second));
        } else if (offsetDay == DAY_SECONDS * 8 ){
            return "8天前 "; //+ noDayFormat.format(new Date(second));
        } else if (offsetDay == DAY_SECONDS * 7 ){
            return "7天前 "; //+ noDayFormat.format(new Date(second));
        } else if (offsetDay == DAY_SECONDS * 6 ){
            return "6天前 "; //+ noDayFormat.format(new Date(second));
        } else if (offsetDay == DAY_SECONDS * 5 ){
            return "5天前 "; //+ noDayFormat.format(new Date(second));
        }else if (offsetDay == DAY_SECONDS * 4 ){
            return "4天前 "; //+ noDayFormat.format(new Date(second));
        }else if (offsetDay == DAY_SECONDS * 3 ){
            return "3天前 "; //+ noDayFormat.format(new Date(second));
        }else if (offsetDay == DAY_SECONDS * 2){
            return "前天 " + noDayFormat.format(new Date(second));
        }else if (offsetDay == DAY_SECONDS ){
            long offset1 = ( System.currentTimeMillis() - second) / 1000;
            if (offset1 > DAY_SECONDS) {
                return "昨天 " + noDayFormat.format(new Date(second));
            } else {
                return (offset1 / HOUR_SECONDS) + "小时前";
            }
        }else {
            long offset2 = ( System.currentTimeMillis() - second) / 1000;
            if (offset2 <= MINUTES) {
                return "刚刚";
            }
            if (offset2 >= HOUR_SECONDS) {
                return (offset2 / HOUR_SECONDS) + "小时前";
            }
            if (offset2 >= MINUTES) {
                return offset2 / MINUTES + "分钟前";
            }
        }
        return "很久以前";
    }
     ...
     * 根据秒数获取时间串  格式：00:00
     */
    public static String getTimeStrBySecond_2(long second) {
        if (second <= 0) {
            return "00:00";
        }
        StringBuilder sb = new StringBuilder();
