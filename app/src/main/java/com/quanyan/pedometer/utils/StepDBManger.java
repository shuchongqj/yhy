...
public class StepDBManger {
    ...
    public static List<WalkDataInfo> loadWalkDataInfoList(long lastStepDayTime,
        ...
        try {
            if (mDbUtils.tableIsExist(WalkDataInfo.class)) {
                List<WalkDataInfo> list = mDbUtils.findAll(Selector.from(WalkDataInfo.class)
    ...
    public static List<WalkDataInfo> loadWalkDataInfoList() {
        ...
        try {
            List<WalkDataInfo> list = new ArrayList<>();
            if (mDbUtils.tableIsExist(WalkDataInfo.class)) {
                ...
                if (list != null && list.size() > 0) {
                    perHour2Day(list, TimeUtil.getTimefromStr(list.get(list.size() - 1).walkDate), TimeUtil.getNowTime());
                }
            }
            // 某日数据为0时补0
    ...
    private static void perHour2Day(List<WalkDataInfo> list, long starttime,
        ...
        List<WalkDataInfoPerHour> tmplist = loadWalkDataInfoPerHourAll(starttime, endtime);
...
//				}
        ...
        if (tmplist != null && tmplist.size() > 0 && list != null
                && list.size() > 0) {
            if (Time.getJulianDay(tmplist.get(0).walkTime, TimeZone.getDefault().getRawOffset() / 1000) ==
                    Time.getJulianDay(list.get(list.size() - 1).walkTime, TimeZone.getDefault().getRawOffset() / 1000)) {
                //本地数据与服务器数据进行目标天数同步
    ...
    private static void fillDayList(List<WalkDataInfo> list, long starttime, long endtime) {
        //后一天的0：00
...
//    }
    ...
    /**
     * @Description 保存saveWalkDataInfoPerHour 保存小时时间到前一毫秒
     * 0:00-0:59 作为一个时间段。
     * 23:00-23:59作为一个时间段。
     * @author xiezhidong@pajk.cn
     * @param stepcount
    ...
    public static boolean saveWalkDataInfoDaily(int stepcount,
                                                double distance, long walkTime, double calories) {
        if (null == mDbUtils) {
            return false;
        }
    ...
    public static class SaveWalkDataInfoPerHourTask extends
        ...
        protected Boolean doInBackground(Void... params) {
            Boolean result;
            if (isdaily) {
                result = saveWalkDataInfoDaily(stepcount, distance,
                        walkTime, calories);
            } else {
                result = saveWalkDataInfoPerHour(stepcount, distance,
                        walkTime, calories);
            }
            return result;
        }
    ...
    public static List<WalkDataInfoPerHour> loadWalkDataInfoPerHourAll(long starttime,
                                                                       long endtime) {
        if (null == mDbUtils) {
            return null;
        }
...
//    }
    ...
    public static class saveAndLoadWalkDataInfoListTask extends
        ...
        public saveAndLoadWalkDataInfoListTask(List<WalkDataInfo> alist, LoadWalkDataInfoListTaskListener lsn) {
            mlist = alist;
            mlsn = lsn;
        }
        ...
        try {
            if (mDbUtils.tableIsExist(WalkDataInfoPerHour.class)) {
                List<WalkDataInfoPerHour> list = mDbUtils.findAll(Selector
                        .from(WalkDataInfoPerHour.class)
                        .where("walkTime", "<=", lastStepDayTime)
                        .and("walkTime", ">", lastSynTime).orderBy("walkTime", true).limit(25 * 6).offset(0));
                return list;
            }
        } catch (DbException e) {
            e.printStackTrace();
    ...
    public static class LoadWalkDataInfoPerHourTask extends
        ...
        protected List<WalkDataInfo> doInBackground(Void... params) {
            List<WalkDataInfo> result = new ArrayList<WalkDataInfo>();
            LogUtils.d("LoadWalkDataInfoPerHourTask mlastStepDayTime:" + mlastStepDayTime
                    + ", mlastSynTime:" + mlastSynTime);
            if (mlastStepDayTime != 0 || mlastSynTime != 0) {
                // 转换服务器更新时需要的结构体
//				perHour2Day( result,mlastStepDayTime, mlastSynTime );
    ...
    public static long getLocalTotleDistance(Context ctx, List<WalkDataInfo> alist) {
