...
public class WebViewActivity extends BaseActivity implements OpenFileChooserCallBack, CameraPopListener, CameraImageListener {
    ...
    private void initData() {
        ...
        mWebView.getSettings().setUserAgentString(ua +
                ...
                ";Channal_" + LocalUtils.getChannel(this));
    ...
    private void callBackJsMethod() {
        ...
        mWebView.registerHandler("getUserInfo", new BridgeHandler() {
            ...
            public void handler(String data, CallBackFunction function) {
                JSONObject jsonObject = new JSONObject();
                try {
                    AccountManager accountManager = AccountManager.Companion.getAccountManager();
                    String phone = accountManager.getPhoneNumber();
                    long user_id = userService.getLoginUserId();
                    String token = accountManager.getUserToken();
        ...
        mWebView.registerHandler("getClientInfo", new BridgeHandler() {
            ...
            public void handler(String data, CallBackFunction function) {
                ...
                try {
                    ...
                    String os = AnalysesConstants.OS_TYPE;
                    ...
                    String adCode = LocationManager.getInstance().getStorage().getLast_discCode();
                    String district = LocationManager.getInstance().getStorage().getLast_discName();
                    jsonObject.put("code", 0);
                    jsonObject.put("message", "OK");
                    JSONObject otherObject = new JSONObject();
                    ...
                    otherObject.put("session", accountManager.getSession());
                    jsonObject.put("data", otherObject.toString());
        ...
        mWebView.registerHandler("share", new BridgeHandler() {
            ...
            public void handler(String data, CallBackFunction function) {
                ...
                try {
                    ...
                    if (mWebParams != null && mWebParams.isArticle()) {
                        // 埋点
                        Analysis.pushEvent(WebViewActivity.this, AnEvent.PageShare,
                                new Analysis.Builder().
                                        ...
                                        setId(String.valueOf(mWebParams.getId())).
                                        setType("文章").
                                        setList(false));
                        // 积分系统
                        ...
                        shareExtraInfo.bizId = mWebParams.getId();
                        shareExtraInfo.bizType = CircleBizType.SNS_ARTICLE;
                        ShareUtils.showShareBoard(WebViewActivity.this, title, desc, url, pic, shareExtraInfo);
                    } else {
                    ...
                    try {
                        if (!TextUtils.isEmpty(data)) {
                            ...
                            if (!MobileUtil.isWeixinAvilible(WebViewActivity.this)) {
                                ToastUtil.showToast(WebViewActivity.this, getString(R.string.wx_isinstall));
                                JSONObject jsonObject = new JSONObject();
                                try {
                                    jsonObject.put("code", 0);
                                    jsonObject.put("message", "OK");
...
//                    String goodsId = object.getString("goodsId");
        ...
        mWebView.registerHandler("articelFinishedComment", new BridgeHandler() {
            ...
            public void handler(String data, CallBackFunction function) {
                try {
                    ...
                    EventBus.getDefault().post(new EvBusNewsCommentChange(ugcid, true));
                } catch (JSONException e) {
        ...
        mWebView.registerHandler("selectCity", new BridgeHandler() {
            ...
            public void handler(String data, CallBackFunction function) {
                ...
                YhyRouter.getInstance().startCitySelectActivity(WebViewActivity.this, true, AnArgs.Instance().build(Analysis.UID, userService.getLoginUserId() > 0 ? userService.getLoginUserId() + "" : "0")
                        .build(Analysis.PAGENAME, "web")
                        .getMap());
            }
        });

        // 自有埋点
        mWebView.registerHandler("statEvent", new BridgeHandler() {
            @Override
            public void handler(String data, CallBackFunction function) {
                try {
                    JSONObject object = new JSONObject(data);
                    String eventCode = object.getString("eventCode");
                    String params = object.getString("params");
                    HashMap<String, String> paramsMap = new HashMap<>();
                    if (!TextUtils.isEmpty(params)) {
                        paramsMap = new Gson().fromJson(params, new TypeToken<HashMap<String, String>>() {
                        }.getType());
                    }
                    Analysis.pushEvent(WebViewActivity.this, eventCode,
                            new Analysis.Builder().
                                    setLat(LocationManager.getInstance().getStorage().getLast_lat()).
                                    setLng(LocationManager.getInstance().getStorage().getLast_lng()).
                                    setMap(paramsMap));
    ...
    public void onBackPressed() {
        if (!TextUtils.isEmpty(eventName)) {
            mWebView.callHandler(eventName, "", new CallBackFunction() {
                @Override
                public void onCallBack(String data) {
                }
            });
            ...
            if (mWebView.canGoBack()) {
                ...
                if (mWebParams != null && mWebParams.isArticle()) {
                    // 埋点
                    Analysis.pushEvent(WebViewActivity.this, AnEvent.PageArticleClose,
                            new Analysis.Builder().
                                    setLat(LocationManager.getInstance().getStorage().getLast_lat()).
                                    setLng(LocationManager.getInstance().getStorage().getLast_lng()).
                                    setId(String.valueOf(mWebParams.getId())));
...
//                return null;
//            }
//            if(!mWebParams.isSetTitle()){
//                return null;
//            }
//
//            mWebParams.setShowTitle(!LocalUtils.isNeedFullScreen(mWebParams.getUrl()));
//
//            if(!mWebParams.isSetTitle()){
//                return null;
//            }
//        }
        //如果URL为空直接关闭页面
        if (StringUtil.isEmpty(mWebParams.getUrl())) {
            finish();
            return null;
        }
        //判断是否为全屏展示
        mNavView = new BaseNavView(this);
        if (!StringUtil.isEmpty(mWebParams.getTitle())) {
    ...
    protected void syncCookie() {
        AccountManager accountManager = AccountManager.Companion.getAccountManager();
        String didCookie = "_did=" + accountManager.getDeviceId();
        ...
        if (userService.isLogin()) {
            ...
            cookieManager.setCookie(DomainUtils.getDomain(), wtkCookie);
            cookieManager.setCookie(DomainUtils.getDomain(), uidCookie);
        }
        cookieManager.setCookie(DomainUtils.getDomain(), didCookie);
        cookieManager.setCookie(DomainUtils.getDomain(), dsigCookie);
...
//        options.setOpenType(OpenType.OPEN_GALLERY_CROP);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == CameraManager.OPEN_USER_ALBUM && resultCode == CameraManager.GET_VIDEO) {
            ...
            ToastUtil.showToast(WebViewActivity.this, getString(R.string.start_upload_video));
        ...
        NetManager.getInstance(this).doUploadImages(files,
                new OnResponseListener<List<String>>() {
                    ...
                    public void onInternError(int errorCode, String errorMessage) {
                        hideLoadingView();
                        ToastUtil.showToast(WebViewActivity.this, errorMessage);
                    }
                    ...
                    public void onComplete(boolean isOK, List<String> result, int errorCode, String errorMsg) {
                        ...
                        if (isOK) {
                            ...
                            ToastUtil.showToast(WebViewActivity.this, errorMsg);
                        }
                    }
                });
    }
...
//                super.onReceivedSslError(view, handler, error);
    ...
    CameraHandler mCameraHandler = new CameraHandler(this, new SelectMoreListener() {
        ...
        public void onSelectedMoreListener(List<MediaItem> pathList) {
            ...
            ToastUtil.showToast(WebViewActivity.this, getString(R.string.start_upload_pic));
            Intent intentService = new Intent(WebViewActivity.this, UpLoadCourseVideoService.class);
            intentService.putExtra(MEDIA_PATH, mVideo.getMediaPath());
            intentService.putExtra(VIDEO_ID, videoId);
    ...
    public void onEvent(EvBusLocationChange evBusLocationChange) {
        if (evBusLocationChange.isFromWeb) {
            if (selectCityFunction == null) return;
            JSONObject jsonObject = new JSONObject();
            try {
                jsonObject.put("code", 0);
                jsonObject.put("msg", "成功");
                JSONObject otherObject = new JSONObject();
                otherObject.put("cityCode", evBusLocationChange.outPlaceCity.cityCode);
                otherObject.put("cityName", evBusLocationChange.outPlaceCity.name);
                jsonObject.put("data", otherObject.toString());
...
//                    && result2 == PackageManager.PERMISSION_GRANTED
    ...
    public static WebView createFastWebView(Context context){
...
//        settings.setAppCacheMaxSize(1024 * 1024 * 8);
        ...
        webView.getSettings().setUserAgentString(ua +
                ";QuanYanYingHeYing_" + AndroidUtils.getVersionCode(context) +
                ";Channal_" + LocalUtils.getChannel(context));
...
//        settings.setCacheMode(WebSettings.LOAD_CACHE_ELSE_NETWORK);
        ...
        webView.setWebChromeClient(new WebChromeClient() {
            ...
            public void onProgressChanged(WebView view, int newProgress) {
                ...
                System.err.println("on page progress changed and progress is " + newProgress);
            }
        });
        webView.loadUrl("https://shadow.yingheying.com/demo/offline-blank/index.html");
        return webView;
    }
}