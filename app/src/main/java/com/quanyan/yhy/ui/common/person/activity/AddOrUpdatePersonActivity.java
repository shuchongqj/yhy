 ...
 * Version 1.0
 */
public class AddOrUpdatePersonActivity extends BaseActivity implements View.OnClickListener, TextWatcher {
    ...
    public static void gotoAddOrUpdatePersonActivity(Activity context, int reqCode, UserContact userContact) {
        Intent intent = new Intent(context, AddOrUpdatePersonActivity.class);
    ...
    protected void initView(Bundle savedInstanceState) {
        ...
        initViews();
        if (userContact == null) {
            mBaseNavView.setTitleText(getString(R.string.title_add_person));
            //sm_title_bar_content.setText(getString(R.string.title_add_person));
            type = TYPE_ADD;
            limitSet(0);
        } else {
            mBaseNavView.setTitleText(getString(R.string.title_update_person));
            //sm_title_bar_content.setText(getString(R.string.title_update_person));
            type = TYPE_EDIT;
            initDatas();
        }
    ...
    private void initDatas() {
        ...
        if (!StringUtil.isEmpty(userContact.certificatesType)) {
            //String idType = showIdType(userContact.certificatesType);
    ...
    private void showSelectIdTypeDialog() {
        InformationDialogUtils.showAlert(AddOrUpdatePersonActivity.this,
    ...
    private void onFinish() {
        ...
        if (idType.equals(getResources().getStringArray(R.array.id_type)[0])) {
            if(!RegexUtil.isIdCard(idValue) || RegexUtil.isBeforOrEnd(idValue)){
            ...
            if(!RegexUtil.isPassport(idValue) || RegexUtil.isBeforOrEnd(idValue)){
            ...
            if(!RegexUtil.isSoldierCard(idValue) || RegexUtil.isBeforOrEnd(idValue)){
                ToastUtil.showToast(this, getString(R.string.soldierCard_error));
                return;
            }
        } else {
            if(!RegexUtil.isHkcard(idValue) || RegexUtil.isBeforOrEnd(idValue)){
        ...
        if (type == TYPE_ADD) {
            ...
            mController.doAddOrUpdateVisitorInfo(AddOrUpdatePersonActivity.this,userContact);
        } else {
            showLoadingView(getString(R.string.edit_finish));
            mController.doUpdateVisitorInfo(AddOrUpdatePersonActivity.this,userContact);
        }
    ...
    public void afterTextChanged(Editable s) {
        ...
        /*if(AddOrUpdatePersonActivity.this.getCurrentFocus() != null){
            if(AddOrUpdatePersonActivity.this.getCurrentFocus().getId() == R.id.et_name){
