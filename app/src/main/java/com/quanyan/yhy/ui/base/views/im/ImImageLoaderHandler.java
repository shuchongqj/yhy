...
public class ImImageLoaderHandler extends Handler {
    ...
    public ImImageLoaderHandler(ImageView imageView, String imageUrl, int errorDrawableRes,
        ...
        if (imageUrl.equals(forUrl)) {
            if (bitmap == null) {
                ...
                if (errorDrawableRes != 0)
                    ...
                    imageView.setBackgroundResource(ImImageView.DEFAULT_ERROR_DRAWABLE_RES_ID);
            } else {
                // add round corner
                if (roundCornerPx > 0 && roundCornerPx <= 100) {
                    bitmap = ImageUtils.getRoundedCornerBitmap(bitmap, roundCornerPx);
                }

                if (imgMaxWidth <= 0 && imgMaxHeight <= 0) {
                    imageView.setImageBitmap(bitmap);
                } else {
                    Bitmap scaledBitmap = ImageUtils.xform(bitmap, imgMaxWidth, imgMaxHeight);
