 ...
 * Time:16:03
 * Version 1.0
 * Description:可按自己想要的格式自动分割显示的EditText
 ...
 */
public class WalletXEditText extends EditText {
    ...
    public WalletXEditText(Context context) {
        this(context, null);
    }

    public WalletXEditText(Context context, AttributeSet attrs) {
        this(context, attrs, android.R.attr.editTextStyle); // Attention !
    }

    public WalletXEditText(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.WalletXEditText, defStyleAttr, 0);

        separator = a.getString(R.styleable.WalletXEditText_x_separator);
        if (separator == null) separator = SPACE;
        customizeMarkerEnable = a.getBoolean(R.styleable.WalletXEditText_x_customizeMarkerEnable, false);
        int which = a.getInt(R.styleable.WalletXEditText_x_showMarkerTime, 0);
        switch (which) {
            case 0:
        ...
        iOSStyleEnable = a.getBoolean(R.styleable.WalletXEditText_x_iOSStyleEnable, false);
        a.recycle();
    ...
    private void init() {
        ...
        if (mRightMarkerDrawable == null) { // 如未设置则采用默认
            mRightMarkerDrawable = getResources().getDrawable(R.mipmap.wallet_edit_clear);
